...
## Faceting
Faceting is the process that split the chart window in several small parts (a grid), and display a similar chart in each section. Each section usually shows the same graph for a specific group of the dataset.
...
```
We can aslo can change the angle, and justification of the axis labels:
...
```{r histogram modify legends 2, echo =TRUE}
ggplot(penguins, aes(flipper_length_mm, fill=species)) + 
  geom_histogram(alpha=0.6, position="identity")+theme_bw()+
  labs(x = "Flipper length (mm)", y = "Counts", fill="Species")+
  theme(axis.text.x = element_text(color = "black", size = 12),
...
- Manual with `scale_color_manual()` or `scale_fill_manual()` (can accept **hex numbers** or **names**):
...
- Creation of [evenly spaced colours for discrete data](https://ggplot2.tidyverse.org/reference/scale_hue.html) with `scale_color_hue()` or `scale_fill_hue()` (can accept hex or names):
...
- Use packages that contain palettes e.g. `RColorBrewer`, `viridis`, or ` Paletteer`:
