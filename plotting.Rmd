...
## Scatterplots
Let's explore if there is a correlation between the **body mass** of the penguins and the **flipper length** of the penguins using [`geom_point()`](https://ggplot2.tidyverse.org/reference/geom_point.html):
...
Let's add the **trend line** (fitting linear model) using [`geom_smooth()`](https://ggplot2.tidyverse.org/reference/geom_smooth.html):
...
Let's add a trend line together with the **equation** and the **R^2^ value** using the package `ggpubr`:
...
Let's try changing themes in other type of plot, histograms using `geom_histogram()`. Let's plot the distribution of the flipper length for each species. We will use Maria's favourite theme, `them_bw()`:
...
And Sarah's favorite theme `theme_linedraw()`:
```{r histogram linedraw theme, echo =TRUE}
...
```{r histogram modify x axis to add more tick marks, echo =TRUE}
#modify axis font size
ggplot(penguins, aes(flipper_length_mm, fill=species)) + 
  geom_histogram(alpha=0.8, position="identity")+theme_bw()+
  labs(x = "Flipper length (mm)", y = "Counts")+
...
```{r modify facet strip.background, echo =TRUE}
ggplot(penguins, aes(x=flipper_length_mm, y=body_mass_g, color=species))+
  geom_point(aes(shape=sex))+
...
For axis scales adjustments and transformations, please see these [examples](http://www.sthda.com/english/wiki/ggplot2-axis-scales-and-transformations).
...
## Dataset
The following material is from the ["Reproducible Reporting with R (R3) for marine ecological indicators" Webminar](https://noaa-iea.github.io/r3-train/visualize.html) designed and instructed by Ben Best, who gracefully allowed us to use it.
...
```
More information on plotly can be found on ["Improving plotly"](https://plotly-r.com/improving-ggplotly.html).
...
```{r, dygraphs stacked, echo=TRUE, fig.height = 4, fig.width = 8}
d_rgn_wide %>%
  dygraph() %>%
  dyOptions(stackedGraph=TRUE)%>%
dyRangeSelector()
```
The example above is rather simple since we used a table that contained time as a numeric variable. If time is a date variable the process is a bit more involved. Information on how to work with dates and how to to get your data at the `date` format can be found [here](https://www.r-graph-gallery.com/316-possible-inputs-for-the-dygraphs-library.html).
...
- [FAQ-reordering](https://ggplot2.tidyverse.org/articles/articles/faq-reordering.html?q=barplot#how-can-i-reorder-the-bars-in-a-bar-plot-by-their-value)
