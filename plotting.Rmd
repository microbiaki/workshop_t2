...
```{r as above but use different shapes for the sexes, echo =TRUE}


ggplot(penguins, aes(x=flipper_length_mm, y=body_mass_g, color=species))+geom_point(aes(shape=factor(sex)))+ geom_smooth(method="lm")
...
ggplot(na.omit(penguins), aes(x=species, y=flipper_length_mm, fill=sex))+geom_violin()+geom_boxplot(position=position_dodge(width=0.9), width=0.2) +theme_bw()
```
